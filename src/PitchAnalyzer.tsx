...
export default function PitchAnalyzer() {
  ...
  // キーボードショートカット:
  //   Space  = 再生/停止
  //   M      = 現在位置にマーカー（Shift+M で波形ピークに吸着）
  //   D      = 現在位置近くのマーカーを削除
  //   ← / →  = コマ送り (1/fps 秒、Shift で 5f)
    ...
    <div style={S.helpRoot}>
      ...
      <ol style={S.helpList}>
        ...
        <li>
          ...
          マーカーをダブルクリックで削除。<br />
          「現在位置に追加」ボタン / M キーでも追加可（Shift 併用で波形ピークに吸着）。
        </li>
        <li>
          <b>ピッチを確認</b>
          <br />
      ...
      <h3 style={S.helpH3}>データの扱い</h3>
      <ul style={S.helpList}>
        <li>
          読み込んだ動画 / 音声ファイルは <b>ブラウザ内だけで処理</b>され、
          サーバーには送信されません。
        </li>
        <li>
          マーカーや解析区間などの操作データもブラウザ内で保持するのみで、
          サーバーには送信せず、保存もしません。タブを閉じれば消えます。
        </li>
        <li>
          mp4 のデコードが失敗した場合のフォールバック処理として、
          ffmpeg のデコーダ本体 (約 25MB) を CDN (jsDelivr) からダウンロードします。
          この際もアップロードされるデータはありません。
        </li>
      </ul>
    </div>
  );
